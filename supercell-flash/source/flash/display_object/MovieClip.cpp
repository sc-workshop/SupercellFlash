...
namespace sc::flash {
        ...
        if (tag == TAG_MOVIE_CLIP_6) {
            ...
            for (uint8_t i = 0; properties_count > i; i++) {
                ...
                switch (property_type) {
                    case 0:
                        custom_properties.emplace_back(std::make_any<bool>(swf.stream.read_bool()));
                        break;
                    default:
        ...
        for (uint16_t i = 0; movieclips_count > i; i++) {
...

            {
                ...
                if (scaling_grid.has_value()) {
                    ...
                    movieclip.scaling_grid = wk::RectF(rectangle->left(), rectangle->top(), rectangle->right(), rectangle->bottom());
                }
            }
            ...
            if (children_names_vector) {
                for (uint16_t c = 0; children_names_vector->size() > c && children_count > c; c++) {
                    movieclip.childrens[c].name = SWFString(strings_vector->Get(children_names_vector->Get(c))->c_str());
                }
            }
            ...
            if (frames_vector) {
                for (auto frame_data : *frames_vector) {
                    ...
                    frame.label = SWFString(strings_vector->Get(label_id)->c_str());
                }
    ...
    void MovieClip::load_compressed(SupercellSWF& swf, const SC2::DataStorage* storage, const uint8_t* data) {
        auto movieclips_data = SC2::GetCompressedMovieClips(data);
