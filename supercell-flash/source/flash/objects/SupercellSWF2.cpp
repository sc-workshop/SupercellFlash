...
namespace sc::flash {
    template <typename T>
    WK_INLINE size_t inline flush_builder(SupercellSWF2CompileTable& table, Offset<T> root_off) {
        table.builder.FinishSizePrefixed(root_off);
        auto buffer = table.builder.GetBufferSpan();
        size_t buffer_size = buffer.size_bytes();
        table.swf.stream.write(buffer.data(), buffer_size);
    ...
    SupercellSWF2CompileTable::SupercellSWF2CompileTable(const SupercellSWF& _swf) :
        swf(_swf) {
    ...
    void SupercellSWF2CompileTable::load_chunk(SupercellSWF& swf,
    ...
    void SupercellSWF2CompileTable::gather_resources() {
        ...
        // Exports
        {
            for (const ExportName& export_name : swf.exports) {
                exports_ref_indices.push_back(get_string_ref(export_name.name, true));
            }
        }
        ...
        // MovieClips
        {
            ...
            for (const MovieClip& movieclip : swf.movieclips) {
                ...
                bool has_children_names =
                    std::any_of(movieclip.childrens.begin(), movieclip.childrens.end(), [](const DisplayObjectInstance& obj) { return !obj.name.empty(); });
                if (has_children_names) {
                    ...
                    for (const DisplayObjectInstance& instance : movieclip.childrens) {
                        children_names->push_back(get_string_ref(instance.name));
                    }
                }
                ...
                for (const MovieClipFrame& frame : movieclip.frames) {
                    frame_labels.push_back(get_string_ref(frame.label));
                }

                std::optional<RefT> scaling_grid = std::nullopt;
                if (movieclip.scaling_grid.has_value()) {
                    scaling_grid = get_rect_ref(movieclip.scaling_grid.value());
                }
    ...
    void SupercellSWF2CompileTable::save_header() {
        ...
        // Matrix Banks
        {
            ...
            for (const MatrixBank& bank : swf.matrixBanks) {
                ...
                for (const flash::ColorTransform& color : bank.color_transforms) {
                    colors.emplace_back(color.multiply.r, color.multiply.g, color.multiply.b, color.alpha, color.add.r, color.add.g, color.add.b);
                }
                ...
                if (swf.sc2_compile_settings.use_half_precision_matrices) {
                    ...
                    for (const flash::Matrix2D& matrix : bank.matrices) {
                        matrices.emplace_back((int16_t) (matrix.a * sm),
                                              (int16_t) (matrix.b * sm),
                                              (int16_t) (matrix.c * sm),
                                              (int16_t) (matrix.d * sm),
        ...
        Offset<SC2::DataStorage> root_off =
            SC2::CreateDataStorage(builder, strings_off, 0, 0, rects_off, movieclip_frame_elements_off, shape_bitmaps_off, banks_off);

        flush_builder(*this, root_off);
    }
    ...
    void SupercellSWF2CompileTable::save_exports() {
        ...
        Offset<SC2::ExportNames> root_off = SC2::CreateExportNamesDirect(builder, &exports_ids, &exports_ref_indices);
    ...
    void SupercellSWF2CompileTable::save_textFields() {
...

        {
            ...
            for (uint32_t i = 0; swf.textfields.size() > i; i++) {
                ...
                SC2::TextField textfield_data(textfield.id,
                                              0,
                                              font_name,
                                              textfield.left,
                                              textfield.top,
        ...
        Offset<SC2::TextFields> root_off = SC2::CreateTextFields(builder, textfields_offs);
    ...
    void SupercellSWF2CompileTable::save_shapes() {
        ...
        Offset<Vector<Offset<sc::flash::SC2::Shape>>> shapes_off;
        {
            ...
            for (const Shape& shape : swf.shapes) {
                ...
                for (const ShapeDrawBitmapCommand& command : shape.commands) {
                    ...
                    commands.emplace_back(0, command.texture_index, (uint32_t) command.vertices.size(), bitmap_data_offset);
                }

                Offset<sc::flash::SC2::Shape> shape_off;
                shape_off = SC2::CreateShapeDirect(builder, shape.id, &commands);
        ...
        Offset<SC2::Shapes> root_off = SC2::CreateShapes(builder, shapes_off);
    ...
    void SupercellSWF2CompileTable::save_movieClips() {
        ...
        Offset<Vector<Offset<sc::flash::SC2::MovieClip>>> movieclips_off;
        {
            ...
            for (uint32_t i = 0; swf.movieclips.size() > i; i++) {
                ...
                std::transform(movieclip.childrens.begin(), movieclip.childrens.end(), std::back_inserter(children_ids), [](const auto& obj) {
                    return obj.id;
                });

                std::transform(movieclip.childrens.begin(), movieclip.childrens.end(), std::back_inserter(children_blending), [](const auto& obj) {
                    return (uint8_t) obj.blend_mode;
                });
                ...
                bool has_frame_lables =
                    std::any_of(movieclip.frames.begin(), movieclip.frames.end(), [](const MovieClipFrame& frame) { return !frame.label.empty(); });

                bool elements_in_short_range = std::any_of(movieclip.frames.begin(), movieclip.frames.end(), [](const MovieClipFrame& frame) {
                ...
                movieclip_off = SC2::CreateMovieClipDirect(builder,
                                                           movieclip.id,
                                                           export_name_ref,
                                                           movieclip.frame_rate,
                                                           movieclip.frames.size(),
                                                           movieclip.unknown_flag,
                                                           children_ids.empty() ? nullptr : &children_ids,
                                                           childrens_names,
                                                           children_blending.empty() ? nullptr : &children_blending,
                                                           frames.empty() ? nullptr : &frames,
                                                           frame_elements_offset,
                                                           movieclip.bank_index,
                                                           scaling_grid,
        ...
        Offset<SC2::MovieClips> root_off = SC2::CreateMovieClips(builder, movieclips_off);
    ...
    void SupercellSWF2CompileTable::save_modifiers() {
        ...
        Offset<SC2::MovieClipModifiers> root_off = SC2::CreateMovieClipModifiers(builder, modifiers_offs);
    ...
    Offset<sc::flash::SC2::TextureData> SupercellSWF2CompileTable::create_texture(const SWFTexture& texture, uint32_t index, bool is_lowres) {
        ...
        result = SC2::CreateTextureData(builder, format, (uint8_t) texture.pixel_format(), target_width, target_height, texture_data_off, external_path_off);
    ...
    uint32_t SupercellSWF2CompileTable::save_textures() {
        Offset<Vector<Offset<SC2::TextureSet>>> textures_offs;
        {
            ...
            for (uint32_t i = 0; swf.textures.size() > i; i++) {
                ...
                texture_set_off = SC2::CreateTextureSet(builder, lowres_texuture_off, highres_texuture_off);
        ...
        Offset<SC2::Textures> root_off = SC2::CreateTextures(builder, textures_offs);

        return (uint32_t) flush_builder(*this, root_off);
    }
