...
namespace sc::flash {
            ...
            if (hashes && exports.empty()) {
                ...
                if (hashmap.size()) {
                    ...
                    for (size_t i = 0; hashmap.size() > i; i++) {
                        ...
                        if (export_name.IsTheEmptyString())
                            continue;

                        auto export_name_hash = values[i].AsBlob();
                        if (export_name_hash.IsTheEmptyBlob())
                            continue;

                        auto& result = CreateExportName(SWFString(export_name.str()), 0);
    ...
    void SupercellSWF::load_sc2(wk::Stream& input) {
        ...
        sc2_compile_settings.use_external_matrix_banks = false;
        if (descriptor->external_matrix_bank_size()) {
            ...
            sc2_compile_settings.use_external_matrix_banks = true;
    ...
    void SupercellSWF::load_sc2_internal(const SC2::Header* descriptor) {
        const SC2::DataStorage* storage = nullptr;
        {
            ...
            if (!sc2_compile_settings.use_external_matrix_banks)
    ...
    void SupercellSWF::save_tags() {
        ...
        if (multi_resolution_suffix.compare(MULTIRES_DEFAULT_SUFFIX) != 0 || low_resolution_suffix.compare(LOWRES_DEFAULT_SUFFIX) != 0) {
    ...
    ExportName* SupercellSWF::GetExportName(const SWFString& name) {
        auto it = wk::find_if_parallel(exports.begin(), exports.end(), [&name](const ExportName& other) { return other.name == name; });
    ...
    uint16_t SupercellSWF::GetDisplayObjectID(const SWFString& name) {
        ...
        throw wk::Exception("Failed to get export name \"%s\"", name.data());
    }

    DisplayObject& SupercellSWF::GetDisplayObjectByID(uint16_t id) {
        auto pred = [&id](DisplayObject& obj) { return obj.id == id; };
