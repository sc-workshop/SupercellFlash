...
namespace sc::flash {
    ...
    bool ColorTransform::operator==(const ColorTransform& color) const {
        if (color.alpha == alpha && color.add.r == add.r && color.add.g == add.g && color.add.b == add.b && color.multiply.r == multiply.r &&
            color.multiply.g == multiply.g && color.multiply.b == multiply.b) {
            return true;
        }
