...
namespace sc::flash {
    ...
    public:
        ...
        SWFString(const std::string& string_data) {
            ...
            string.erase(std::remove_if(string.begin(), string.end(), [](char value) { return !SWFString::IsValid(value); }), string.end());
    ...
    public:
        ...
        uint8_t length() const { return (uint8_t) m_data.length(); }

        char* data() const { return (char*) m_data.data(); }

        std::string string() const { return m_data; }

        void clear() { m_data.clear(); }

        int compare(const char* string) const { return m_data.compare(0, std::string::npos, string); }

        int compare(const std::string& string) const { return m_data.compare(string); }
        ...
        bool operator==(const SWFString& other) const { return compare(other.data()) == 0; }
    ...
    public:
        ...
        static bool IsValid(char ch) { return (ch >= 32 && ch <= 126) || ch == '\n' || ch == '\t'; }
